...
dnn_model = Dnn.build(width = SIZE,
                      heigth = SIZE,
                      depth = CHANNEL,
                      classes = train_generator.num_classes)

opt = SGD(learning_rate = 0.001, momentum = 0.1)

dnn_model.compile(optimizer = opt, loss = 'categorical_crossentropy',
              metrics = ['acc'])
...
h = dnn_model.fit(train_generator, epochs = EPOCHS, verbose = 1,
...
dnn_model.save('outputs/trained_models/' + FILE_NAME + file_date + '.h5')

##############################################################################

print('[INFO] Avaliando modelo com conjunto de teste...\n')

loss, acc = dnn_model.evaluate(test_generator)
...
Y_pred = dnn_model.predict(test_generator, test_steps_per_epoch)
...
plot_model(dnn_model, to_file='outputs/models_architecture_images/' + FILE_NAME + file_date + '.png',
          show_shapes=True)
